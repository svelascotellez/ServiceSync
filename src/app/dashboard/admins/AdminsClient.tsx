            ...
            <tr style={{ borderBottom: '2px solid var(--border)', backgroundColor: 'var(--surface-hover)', userSelect: 'none' }}>
              <th style={{ padding: '0.85rem 1rem' }}>
                <ExcelColumnHeader 
                  title="Usuario"
                  columnKey="name"
                  uniqueValues={Array.from(new Set(admins.map(a => a.name))).filter(Boolean)}
                  selectedValues={[]}
                  onFilterChange={() => {}}
              ...
              <th style={{ padding: '0.85rem 1rem' }}>
                <ExcelColumnHeader 
                  title="Rol"
                  columnKey="role"
                  uniqueValues={['admin', 'supervisor']}
                  selectedValues={roleFilter === 'all' ? [] : [roleFilter]}
                  onFilterChange={(sel) => setRoleFilter(sel.length === 1 ? sel[0] : 'all')}
                  currentSort={{ column: sortColumn, order: sortOrder }}
                  onSortChange={(col, order) => { setSortColumn(col); setSortOrder(order); }}
                  displayFormatter={(r) => r === 'admin' ? 'Administrador' : 'Supervisor'}
                />
              </th>
              <th style={{ padding: '0.85rem 1rem' }}>
                <ExcelColumnHeader 
                  ...
                  columnKey="email"
                  uniqueValues={Array.from(new Set(admins.map(a => a.email))).filter(Boolean)}
                  selectedValues={[]}
                  onFilterChange={() => {}}
                  currentSort={{ column: sortColumn, order: sortOrder }}
                  onSortChange={(col, order) => { setSortColumn(col); setSortOrder(order); }}
              ...
              <th style={{ padding: '0.85rem 1rem' }}>
                <ExcelColumnHeader 
                  title="Teléfono"
                  columnKey="phone"
                  uniqueValues={Array.from(new Set(admins.map(a => a.phone || 'N/A'))).filter(Boolean)}
                  selectedValues={[]}
                  onFilterChange={() => {}}
                  currentSort={{ column: sortColumn, order: sortOrder }}
                  onSortChange={(col, order) => { setSortColumn(col); setSortOrder(order); }}
          ...
          <tbody>
            {filteredAdmins.map(admin => (
              <tr key={admin.id} style={{ borderBottom: '1px solid var(--border)', transition: 'background-color 0.15s ease' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'var(--surface-hover)'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                ...
                <td style={{ padding: '0.85rem 1rem', color: 'var(--text-secondary)' }}>{formatCancunDate(admin.createdAt)}</td>
